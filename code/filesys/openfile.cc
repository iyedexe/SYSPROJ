...
{ 
    ...
    if(sector != 0 && sector != 1) //map and directory
    {
        fileSystem->addFile(sector); 
    }
}
...
OpenFile::~OpenFile()
{
    if(hdr->getHdrSector() != 0 && hdr->getHdrSector() != 1) //map and directory
    {
        fileSystem->rmFile(hdr->getHdrSector()); 
    }
    delete hdr;
}
...
FileHeader* 
OpenFile::getFileHeader(){return hdr;}
